...
var createChecksum = function (string) {
...
AuthenticationStrategy.prototype.validateToken = function (user, next) {
	this.db.collection(this.options.collection).findOne({_id: user.name}, function (error, userDocument) {
		if (error || !userDocument) {
			...
			var checksum = createChecksum(userDocument.password);
			return next(null, checksum && user.checksum == checksum);
		}
	});
};
...
AuthenticationStrategy.prototype.createTokenData = function (user, next) {
	this.db.collection(this.options.collection).findOne({_id: user.name}, function (error, userDocument) {
		if (error || !userDocument) {
			...
			var tokenData = _.pick(userDocument, ['name', 'email']);
			tokenData.checksum = createChecksum(userDocument.password);
